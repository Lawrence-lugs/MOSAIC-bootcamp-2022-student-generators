...
class layout(AnalogBase):
    ...
    **kwargs :
    ...
    def draw_layout(self):
        ...
        rows = RowList([row_Dummy_NB, row_nmos, row_Dummy_NT, row_Dummy_PB, row_pmos, row_Dummy_PT])
        ...
        nmos = Transistor(name='n', row=row_nmos, fg=fg_n, seff_net='VSS', deff_net='VREF')
        pmos = Transistor(name='p', row=row_pmos, fg=fg_p, seff_net='VDD', deff_net='VREF')
        ...
        tid_nmos_G = self.get_wire_id('nch', row_nmos_idx, 'g', wire_name='OUT')
        tid_pmos_G = self.get_wire_id('pch', row_pmos_idx, 'g', wire_name='TG')
        tid_nmos_S = self.get_wire_id('nch', row_nmos_idx, 'ds', wire_name='TG')
        tid_pmos_S = self.get_wire_id('pch', row_pmos_idx, 'ds', wire_name='TV')
        tid_pmos_D = self.get_wire_id('pch', row_pmos_idx, 'ds', wire_name='VREF')

        # 7:  Perform wiring
        warr_p_G = self.connect_to_tracks([pmos.g], tid_pmos_G)
        warr_p_S = self.connect_to_tracks([pmos.s], tid_pmos_S)
        warr_n_S = self.connect_to_tracks([nmos.s], tid_nmos_S)
        # only need one of these since they're connected to each other.
        warr_p_D = self.connect_to_tracks([pmos.d, nmos.d, nmos.g], tid_pmos_D)
