...
#       17) CS Amp with diode load Layout Generator      #
...
# 3. Create instances.
      ...
      in1 = tnmos.generate(name='MN1', transform='MX', params={'nf': nf, 'tie': 'S'})
      
# 4. Place instances.
      dsn.place(grid=pg, inst=in0, mn=[0,0])
      dsn.place(grid=pg, inst=in1, mn=pg.mn.top_left(in0) + pg.mn.height_vec(in1))

# 5. Create and place wires.
      print("Create wires")
      
      # OUT
      _mn = [r23.mn(in1.pins['D'])[1], r23.mn(in1.pins['G'])[1], r23.mn(in0.pins['D'])[1]]
      ...
      # VDD
      rvdd0 = dsn.route(grid=r12, mn=[r12.mn(in1.pins['RAIL'])[0], r12.mn(in1.pins['RAIL'])[1]])
      
# 6. Create pins.
      pin0 = dsn.pin(name='I', grid=r12, mn=r12.mn.bbox(in0.pins['G']))
      pout0 = dsn.pin(name='O', grid=r23, mn=r23.mn.bbox(rout1))
